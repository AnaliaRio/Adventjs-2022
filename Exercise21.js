...
function printTable(gifts) {
  ...
  const secondColumnSize = Math.max(
    "Quantity".length,
    ...gifts.map((gift) => gift.quantity.toString().length)
  );

  // Calculate table size
  const tableSize = firstColumnSize + secondColumnSize + 7;

  // Header of the table
  let tableString =
    `${"+".repeat(tableSize)}\n` +
    `| ${"Gift".padEnd(firstColumnSize)} | ` +
    `${"Quantity".padEnd(secondColumnSize)} |\n` +
    `| ${"-".repeat(firstColumnSize)} | ${"-".repeat(secondColumnSize)} |\n`;

  // Body of the table
  gifts.forEach(({ name, quantity }) => {
    tableString += `| ${name.padEnd(firstColumnSize)} `;
    tableString += `| ${(quantity + "").padEnd(secondColumnSize)} |\n`;
  });

  //Closing table
  tableString += "*".repeat(tableSize);
