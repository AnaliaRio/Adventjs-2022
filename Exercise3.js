/*
deliver to the children. Each gift inside the pack is represented by a string and it has a weight equal to the number of letters in its name. Santa Claus's sleigh can only carry a weight limit.
...
function distributeGifts(packOfGifts, reindeers) {
  const giftsWeigth = packOfGifts.reduce(
    (weightAcc, gift) => weightAcc + gift.length,
    0
  );
  const reindeersCarry = reindeers.reduce(
    (weightCarryAcc, reindeer) => weightCarryAcc + reindeer.length * 2,
    0
  );
